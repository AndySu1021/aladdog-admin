...
const rules = reactive({
  ...
  title: [{ required: true, message: '請輸入標題', trigger: 'blue' }],
  type: [{ required: true, message: '請選擇類型', trigger: 'change' }],
  date_range: [{ required: true, message: '請選擇日期', trigger: 'change' }]
})
</script>
  ...
  >
    ...
    >
      ...
      <ElFormItem
        ...
        label="折扣金額"
        required
        prop="condition.discount_amount"
      >
        <ElInput
      ...
      <ElFormItem
        ...
        label="數量"
        required
        prop="condition.quantity"
      >
        <ElInputNumber v-model.number="form.condition.quantity" :min="0" />
      </ElFormItem>
    </ElForm>
  </AppDrawer>
</template>
