  ...
  <div class="dddd">
    <AppDialog ref="DetailDialog" :title="`訂單詳情 #${detailData.order_no}`" width="800px">
      <ElDescriptions class="margin-top" :column="2" size="large" border>
        ...
        <el-descriptions-item label="批次號" label-align="center">{{
          detailData.batch_no
        }}</el-descriptions-item>
        <el-descriptions-item label="入桌時間" label-align="center">{{
          detailData.start_time
        }}</el-descriptions-item>
        <el-descriptions-item label="離桌時間" label-align="center">{{
          detailData.end_time
        }}</el-descriptions-item>
        <el-descriptions-item label="用餐人數" label-align="center"
          >{{ detailData.headcount }} 人</el-descriptions-item
        >
        <el-descriptions-item label="總金額" label-align="center">
          {{ formatter.format(detailData.total_amount) }}
        </el-descriptions-item>
      </ElDescriptions>
      <ElTable style="margin-top: 24px" :data="detailData.sub_orders" border>
        <ElTableColumn type="expand">
          <template #default="scope">
            <div style="width: 90%; margin: auto">
              <ElTable row-key="id" :data="scope.row.products" border>
