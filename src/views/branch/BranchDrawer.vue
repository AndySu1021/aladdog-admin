...
const rules = reactive({
  ...
  code: [
    { required: true, message: '請輸入代號', trigger: 'blue' },
  ],
  tax_id_number: [
    { required: true, message: '請輸入統一編號', trigger: 'blue' },
  ],
  address: [
    { required: true, message: '請輸入地址', trigger: 'blue' },
  ],
  minimum_order: [
    { required: true, message: '請輸入低消', trigger: 'blue' },
  ],
  password: [
      ...
      <ElFormItem label="地址" required prop="address">
        ...
        <ElRow :gutter="8" style="">
          <ElCol :sm="24" :md="12" :lg="8">
            <ElSelect v-model="form.city" value-key="name" placeholder="請選擇" @change="handleCity">
              <ElOption v-for="(city,idx) in addressData" :key="idx" :label="city.name" :value="{...city, idx: idx}" />
            </ElSelect>
          </ElCol>
          <ElCol :sm="24" :md="12" :lg="8">
            <ElSelect v-model="form.district" value-key="name" placeholder="請選擇" @change="handleDistrict">
              <ElOption v-for="(district,idx) in addressData[cityId].districts" :key="idx" :label="district.name" :value="{...district, idx: idx}" />
            </ElSelect>
          </ElCol>
          <ElCol :sm="24" :md="12" :lg="8">
